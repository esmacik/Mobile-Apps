...
public class MainActivity extends AppCompatActivity {
    ...
    //Populates global array of blue tile bitmaps
    private void getBlueTiles(Bitmap tileImage) {
        int x = 0;
        int y = 0;
        int height = 108;
        int width = 108;
        for (int i = 0; i < blueTile.length; i++) {
            blueTile[i] = Bitmap.createBitmap(tileImage, x, y, width, height);
            x += 108;
        }
    }

    //Populates global array of green tile bitmaps
    private void getGreenTiles(Bitmap tileImage) {
        int x = 0;
        int y = 216;
        int height = 108;
        int width = 108;
        for (int i = 0; i < greenTile.length; i++) {
            greenTile[i] = Bitmap.createBitmap(tileImage, x, y, width, height);
            x += 108;
        }
    }
